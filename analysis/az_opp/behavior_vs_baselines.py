...
for i in range(N):
    *_, mask, tgt = val_ds[i]
    ...
    k = int(legal.sum()); n_legal_sum += k
    uni_ce += np.log(k)                                   # uniform over legal
    lp = log_marg.copy(); lp[~legal] = NEG
    lp = lp - np.log(np.exp(lp - lp.max()).sum()) - lp.max()*0  # renorm in logspace
    # proper masked renorm:
    z = np.log(np.exp(log_marg[legal] - log_marg[legal].max()).sum()) + log_marg[legal].max()
...
bs=2048; ce=0.0; top1=top3=tot=0
