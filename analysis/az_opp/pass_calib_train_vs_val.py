...
def pass_calib(ds, tag):
    ...
    with torch.no_grad():
        for hand, opp, trick, osz, usz, value, mask, tgt in DataLoader(
            ...
            P += list(pi[sel, PASS])
            Y += list((tgt.numpy()[sel] == PASS).astype(float))
    P = np.array(P)
    Y = np.array(Y)
    ...
    for lo, hi in [(0, 0.1), (0.1, 0.3), (0.3, 0.5), (0.5, 0.7), (0.7, 1.01)]:
        ...
        am = Y[b].mean()
        pm = P[b].mean()
        se = np.sqrt(max(am * (1 - am), 1e-9) / b.sum())
        print(
            f"  [{lo:.1f},{hi:.1f}) pred={pm:.3f} actual={am:.3f}±{1.96*se:.3f} (n={int(b.sum())})"
        )


pass_calib(train, "TRAIN")
pass_calib(val, "VAL")
