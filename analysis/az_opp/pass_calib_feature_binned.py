...
NONBOMB_SLOTS = [i for i in range(len(val_ds[0][6]))
                 if i != PASS_SLOT and i not in set(BOMB_SLOTS)]
...
Ppass = np.array(Ppass); Ypass = np.array(Ypass)
Pbomb = np.array(Pbomb); Ybomb = np.array(Ybomb)
