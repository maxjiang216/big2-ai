...
with torch.no_grad():
    for hand, opp, trick, osz, usz, value, mask, tgt in dl:
        ...
        sel = mask[:, PASS_SLOT]
        P_pass += list(pi[sel, PASS_SLOT])
        Y_pass += list((tgt[sel] == PASS_SLOT).astype(float))
        ...
        P_bomb += list(pb[bmask])
        Y_bomb += list(yb[bmask])
...
def report(name, P, Y):
    ...
    nneg = n - npos
    auc = (
        (r[Y == 1].sum() - npos * (npos + 1) / 2) / (npos * nneg)
    ...
    for lo in (0, 0.1, 0.3, 0.5, 0.7, 0.9):
        hi = lo + (0.1 if lo == 0 else (0.2 if lo < 0.9 else 0.1))
        b = (P >= lo) & (P < hi)
        if b.sum():
            print(f"    [{lo:.1f},{hi:.1f}) -> {Y[b].mean():.3f}  (n={int(b.sum())})")
