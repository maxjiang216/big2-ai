#!/usr/bin/env python3
"""
Last-player position analysis from turn_features.parquet.

Tablebase aggregates (``tb_case1``, ``tb_case2``, ``tb_forced_seq_len``,
``tb_opp1_table_straight``, ``tb_hits``) are **game-level** columns in
``game_features.parquet``, not turn-level. This script only reads turn parquet;
merge game features if you need those columns in analysis.
"""
