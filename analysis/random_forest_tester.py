...
Turn parquet columns are taken from ``generate_data`` output.
Numeric columns are used as features except ``turn_outcome`` and ``next_player``.
Tablebase columns are game-level in ``game_features.parquet``; they are not
present in turn-only exports unless you merge them in.
"""
