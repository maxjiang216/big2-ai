...
print(
    f"gen0 argmax==pass(0): {pass_frac(arg0):.3f}   gen1 argmax==pass(0): {pass_frac(arg1):.3f}"
)
...
print(
    f"gen0 distinct argmax moves: {len(u0)}; top move {u0[c0.argmax()]} chosen {c0.max()/len(sample):.3f}"
)
print(
    f"gen1 distinct argmax moves: {len(u1)}; top move {u1[c1.argmax()]} chosen {c1.max()/len(sample):.3f}"
)
print(
    "gen1 top-5 argmax moves (id:frac):",
...
if vis_idx:
    tgt = np.array(
        [
            sample[i]["visit_moves"][int(np.argmax(sample[i]["visit_counts"]))]
            for i in vis_idx
        ]
    )
    a1 = arg1[vis_idx]
...
print(
    f"mean log(#legal) [uniform entropy ceiling]: {float(nlegal.log().mean()):.4f} nats"
)

print("\n=== Value head ===")
print(
    f"gen0 value: mean {v0.mean():.4f} std {v0.std():.4f} min {v0.min():.4f} max {v0.max():.4f}"
)
print(
    f"gen1 value: mean {v1.mean():.4f} std {v1.std():.4f} min {v1.min():.4f} max {v1.max():.4f}"
)
...
print(
    f"gen0 head: mean {h0.mean():.3f} std {h0.std():.3f} per-dim-std(min/mean/max) "
    f"{h0.std(0).min():.3f}/{h0.std(0).mean():.3f}/{h0.std(0).max():.3f}"
)
print(
    f"gen1 head: mean {h1.mean():.3f} std {h1.std():.3f} per-dim-std(min/mean/max) "
