...
NNEvaluator::eval_opps(const std::vector<OppFeatures> &feats) const {
  ...
  auto out = opp_
                 .forward({t_hand.to(device_), t_opp.to(device_),
                           ...
                           t_usz.to(device_)})
                 .toTuple();
  auto move_value = out->elements()[0].toTensor().to(torch::kCPU).contiguous();
  auto logits = out->elements()[1].toTensor().to(torch::kCPU).contiguous();
  const float *mv = move_value.data_ptr<float>();
  const float *lp = logits.data_ptr<float>();

  std::vector<OppEval> result(B);
  for (int i = 0; i < B; ++i) {
    std::copy(mv + i * AZ_OPP_HEAD_DIM, mv + (i + 1) * AZ_OPP_HEAD_DIM,
              result[i].move_value.begin());
