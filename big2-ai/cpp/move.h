...
public:
    ...
    Move(Combination comb, int r = 0, int aux = 0) : combination(comb), rank(r), auxiliary(aux) {}
};

// Encode a move as an integer in the range [0, TOTAL_MOVES-1].
// (Legal moves are linear‐indexed as follows: 0 = pass, then 13 singles, 12 doubles, 11 triples,
// 132 full houses, 156 bombs, 54 straights, 56 sisters and 38 triple straights.)
int encodeMove(const Move &move);

// Decode an encoded integer into a Move.
