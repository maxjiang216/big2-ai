// highest_double_not_bomb_feature.h

#ifndef HIGHEST_DOUBLE_NOT_BOMB_FEATURE_H
#define HIGHEST_DOUBLE_BOT_BOMB_FEATURE_H
...
class HighestDoubleNotBombFeature : public FeatureExtractor {
public:
  Type type() const override { return Type::TurnLevel; }
  std::string name() const override { return "highest_double_not_bomb"; }
    ...
    for (const auto &turn : turns) {
      for (int perspective = 0; perspective < 2; ++perspective) {
        ...
        for (int i = 11; i >= 0; --i) { // from 2 down to 3
          if (hand[i] > 1 && hand[i] < 4 && (i < 12 || hand[i] < 3)) {
            highest_rank = i;
            break;
          }
        }
...
#endif // HIGHEST_DOUBLE_NOT_BOMB_FEATURE_H
