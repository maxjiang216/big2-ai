// highest_triple_not_bomb_feature.h

#ifndef HIGHEST_TRIPLE_NOT_BOMB_FEATURE_H
#define HIGHEST_TRIPLE_NOT_BOMB_FEATURE_H
...
class HighestTripleNotBombFeature : public FeatureExtractor {
public:
  Type type() const override { return Type::TurnLevel; }
  std::string name() const override { return "highest_triple_not_bomb"; }
  ...
  std::vector<int> turnExtract(const GameRecord &record) const override {
    ...
    for (const auto &turn : turns) {
      for (int perspective = 0; perspective < 2; ++perspective) {
        ...
        for (int i = 10; i >= 0; --i) { // from 2 down to 3
...
#endif // HIGHEST_TRIPLE_NOT_BOMB_FEATURE_H
