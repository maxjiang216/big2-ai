...
#include "card.h"       // for Card::rank or similar accessor
#include "move.h"
#include <array>
#include <stdexcept>
...
RandomPlayer::RandomPlayer(unsigned int seed)
    : rng_(seed),
      game_() {}

// Accept the initial deal: reset game state with this hand
void RandomPlayer::accept_deal(const std::vector<Card>& hand, bool turn) {
    std::array<int, 13> counts{};
    for (const auto& card : hand) {
        int idx = card.rank - 3;  // assuming rank ∈ {3..15}
        if (idx < 0 || idx >= 13) {
            throw std::out_of_range("Card rank out of bounds");
        }
        counts[idx]++;
    }
    // Initialize PartialGame
    game_ = PartialGame(counts, turn);
}
