...
#          [--device cuda|cpu] [--seed N] [--g0-model PATH]
...
EVAL_DEALS=1000
EVAL_SLOTS=2000
BATCH=512            # GAMES per train batch (~20+ sample rows each)
LR=3e-4
...
DEVICE=cuda
SEED=42
G0_MODEL=models/az_seq_gen0.pt   # shared teacher-bootstrapped gen0 (seq net)
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --seed)       SEED=$2; shift 2 ;;
    --g0-model)   G0_MODEL=$2; shift 2 ;;
    *) echo "unknown arg: $1" >&2; exit 1 ;;
  esac
done

mkdir -p "$LOG_DIR" data models
[[ -f "$G0_MODEL" ]] || { echo "missing gen0 model $G0_MODEL" >&2; exit 1; }
...
# batched eval of model A at sims SA vs a panel member -> prints summary line
eval_vs() {  # eval_vs <model> <sims> <label> <opp-args...>  (writes to global $OUTF)
  local ma=$1 sa=$2 label=$3; shift 3
  bin/eval_az_match --model-a "$ma" --sims "$sa" \
...
run_one() {  # run_one <sims>
  local S=$1 tag="s${1}"
  local cm="models/az_seq_${tag}_champion.pt"
  local LOG="$LOG_DIR/run_${tag}.log"
  : >"$LOG"
  cp "$G0_MODEL" "models/az_seq_${tag}_gen0.pt"
  cp "$G0_MODEL" "$cm"
  ...
  for GEN in $(seq 1 "$GENS"); do
    ...
    local gd="data/az_${tag}_games_gen${GEN}.parquet"
    local m="models/az_seq_${tag}_gen${GEN}.pt"
    ...
    bin/az_selfplay --games "$GAMES" --sims "$S" --slots "$SLOTS" --device "$DEVICE" \
        --seed "$((SEED + GEN))" \
        --model "$cm" \
        --player-out "$pd" --opp-out "$od" --games-out "$gd" 2>&1 | grep -E 'samples:' | tee -a "$LOG"
    echo "    self-play ${S}-sims: $(( $(date +%s) - T0 ))s" | tee -a "$LOG"

    local GF="$gd" PF="$pd" OF="$od" i g
    for i in $(seq 1 "$MIX_GENS"); do
      g=$((GEN - i))
      if [[ "$g" -ge 1 && -f "data/az_${tag}_player_gen${g}.parquet" ]]; then
        GF="$GF data/az_${tag}_games_gen${g}.parquet"
    ...
    uv run python -m nn.train_az_seq --games-data $GF --player-data $PF --opp-data $OF \
        --out "$m" \
        --sched plateau --ckpt-metric head --epochs "$EPOCHS" --batch-games "$BATCH" --lr "$LR" \
    ...
    eval_vs "$m" "$S" random        --classic random        | tee -a "$LOG"
    eval_vs "$m" "$S" greedy        --classic greedy        | tee -a "$LOG"
    eval_vs "$m" "$S" "pimc(20)"    --classic pimc --classic-param 20 | tee -a "$LOG"
    eval_vs "$m" "$S" typed_search  --classic typed_search  | tee -a "$LOG"
    eval_vs "$m" "$S" champion      --model-b "$cm" | tee -a "$LOG"
  ...
  rm -f "$OUTF"
  echo "════ RUN sims=$S done. champion = $cm ════" | tee -a "$LOG"
}
...
for ((i=0;i<${#SARR[@]};i++)); do
  for ((j=i+1;j<${#SARR[@]};j++)); do
    SA=${SARR[$i]} SB=${SARR[$j]}
    bin/eval_az_match \
      --model-a "models/az_seq_s${SA}_champion.pt" \
      --model-b "models/az_seq_s${SB}_champion.pt" \
