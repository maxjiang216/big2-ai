...
def export_tree(joblib_path: str, txt_path: str) -> None:
    ...
    with open(txt_path, "w") as f:
        ...
        for i in range(n_nodes):
            f.write(
                f"{features[i]} {thresholds[i]:.8f} {left[i]} {right[i]} {values[i]:.8f}\n"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--depth", type=int, default=None, help="Export model for a specific depth"
    )
    parser.add_argument(
        "--all", action="store_true", help="Export all tree_model_d*.joblib files found"
    )
    parser.add_argument(
        "--model-dir", default="data", help="Directory containing .joblib model files"
    )
    parser.add_argument(
        "--output-dir", default="data", help="Directory to write .txt model files"
    )
    ...
    if args.all:
        ...
        for p in paths:
            depth = (
                os.path.basename(p).removeprefix("tree_model_d").removesuffix(".joblib")
            )
